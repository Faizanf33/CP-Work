/*
Q6) Write a program that declares a struct to store the data of a football
...
and the number of rushing yards). Declare an array of 10 components to store
the data of 10 football players. Your program must contain a function to
input data and a function to output data. Add functions to search the array
to find the index of a specific player, and update the data of a player.
(You may assume that input data is stored in a file.)
...
bool getData(string file, Player player[], int size){
	...
	if (fin){
		for (int i=0; i<size; i++){
			fin >> player[i].name >> player[i].position >>
			player[i].touchdowns >> player[i].catches >>
			player[i].passing_yards >> player[i].receiving_yards >>
...
bool outData(string file, Player player[], int size){
	...
	for (int i=0; i<size; i++){
		fout << player[i].name << " " << player[i].position <<
		" " << player[i].touchdowns << " " << player[i].catches <<
		" " << player[i].passing_yards << " " << player[i].receiving_yards <<
