/*
Q3) Write a program to help a local restaurant automate its breakfast billing
...
void printCheck(menuItemType menuList[], int size, int order[]){
	...
	for (int i=0; i<size; i++){
		if (order[i] != 0){
			...
			cout << fixed << setprecision(2) << setw(9) << right <<
			"$" <<menuList[i].charges;
	...
	cout << setw(25) << left <<"|TOTAL TAX " <<
	 setw(8) << setw(10) << fixed << right << setprecision(2) << "$" <<
	 tax << "\t\t\t|" << endl;
...
void showMenu(menuItemType menuList[], int size, int order[]){
	...
	for (int i=0; i<size; i++){
		if (menuList[i].quantity > 0){
			...
			cout << fixed << setprecision(1) << setw(7) << right <<
			menuList[i].tax;
